...
{
	da->format(da->arg, "%s", da->spec->mnemonic);
}

static void format_op(int n, struct m68kda *da)
{
	da->format(da->arg, "%s", !n ? "\t" : ",");
}

static void format_d(uint8_t d, struct m68kda *da)
{
	da->format(da->arg, "%s", data_register_symbol(d));
}

static void format_a(uint8_t a, struct m68kda *da)
{
	da->format(da->arg, "%s", address_register_symbol(a));
}
