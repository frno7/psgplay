...
void write_function_name(FILE* filep, char* base_name)
{
	fprintf(filep, "static void %s(struct m68k_module *module)\n", base_name);
}
...
void generate_opcode_handler(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* opinfo, int ea_mode)
{
	...
	{
		sprintf(str, "EA_%s_8(module)", g_ea_info_table[ea_mode].ea_add);
		add_replace_string(replace, ID_OPHANDLER_EA_AY_8, str);
		sprintf(str, "EA_%s_16(module)", g_ea_info_table[ea_mode].ea_add);
		add_replace_string(replace, ID_OPHANDLER_EA_AY_16, str);
		sprintf(str, "EA_%s_32(module)", g_ea_info_table[ea_mode].ea_add);
		add_replace_string(replace, ID_OPHANDLER_EA_AY_32, str);
		sprintf(str, "OPER_%s_8(module)", g_ea_info_table[ea_mode].ea_add);
		add_replace_string(replace, ID_OPHANDLER_OPER_AY_8, str);
		sprintf(str, "OPER_%s_16(module)", g_ea_info_table[ea_mode].ea_add);
		add_replace_string(replace, ID_OPHANDLER_OPER_AY_16, str);
		sprintf(str, "OPER_%s_32(module)", g_ea_info_table[ea_mode].ea_add);
...
int main(int argc, char **argv)
{
	...
	char filename[M68K_MAX_DIR + M68K_MAX_PATH];
	/* Section identifier */
