...
static bool sndh_substrings_subtag(struct sndh_cursor  *cursor)
{
	...
	/* keep track of the biggest string-end seen */
	if (e > cursor->subtag.max_e)
		cursor->subtag.max_e = e;
	...
	if (cursor->offset == cursor->subtag.bound)
		cursor->offset = cursor->subtag.bound = cursor->subtag.max_e;
